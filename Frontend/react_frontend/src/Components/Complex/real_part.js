...
const RealPart = () => {
    ...
    if (input_data.data) {
        try {
            result = nerdamer(`realpart(${input_data.data})`).toString()
        }
        catch {
        ...
        <div className="container-sm">
            <div className="row">
                ...
                <div className="row card bg-dark mx-auto">
                    ...
                    {result !== 'Some confusion in your input' && <motion.div animate={{ y: 10 }} className='text-info fw-bold rounded  fs-3 card-body text-start'>{result}</motion.div>}
                </div>
            </div>
        </div>
    )
